...
from src.schema.userschema import LoginRequest, UserCreate, UserResponse
...
async def register(user: UserCreate, db: Session = Depends(get_db), response: Response = None):
    if db.query(User).filter(User.email == user.email).first():
        raise HTTPException(status_code=400, detail="Tвой логин yжe существует")

    try:
        role_str = user.role.lower()
        role = UserRole(role_str)
    except ValueError:
        raise HTTPException(status_code=400, detail="не правильная роль")
